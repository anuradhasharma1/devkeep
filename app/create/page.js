    ...
    return (
        <div style={{ position: "relative" }}>
            <div
                ...
                style={{
                    display: "flex", flexWrap: "wrap", gap: ".35rem", minHeight: 42,
                    padding: ".5rem .75rem", border: "1px solid var(--border)", borderRadius: 5,
                    background: "var(--card)", cursor: "text", alignItems: "center",
            ...
            >
                {tags.map(t => (
                    <span key={t} style={{
                        ...
                        borderRadius: 999, padding: "2px 10px", fontSize: ".62rem", color: "var(--accent)",
                    }}>
                        #{t}
                        <button type="button" onClick={() => removeTag(t)}
                            style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--accent)", fontSize: ".75rem", lineHeight: 1, padding: 0 }}>
                            ×
                        </button>
                    </span>
                ))}
                <input
                    ...
                    style={{
                        border: "none", background: "transparent", fontFamily: "'IBM Plex Mono', monospace",
                        fontSize: ".72rem", color: "var(--text)", outline: "none", minWidth: 80, flex: 1,
                    }}
                />
            </div>
...
function PreviewCard({ title, description, code, language, tags, isPublic }) {
    return (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 9, overflow: "hidden" }}>
            ...
            <pre style={{
                margin: ".25rem 1rem .9rem", background: "var(--bg)", borderRadius: 5,
                padding: ".85rem 1rem", border: "1px solid var(--border)",
                fontSize: ".65rem", lineHeight: 1.75, color: "var(--accent)",
                minHeight: 70, whiteSpace: "pre-wrap", overflow: "hidden",
                maxHeight: 140, position: "relative", fontFamily: "'IBM Plex Mono', monospace",
            }}>
...
export default function CreatePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(true);
        try {
            await addSnippet({ title: title.trim(), description: description.trim(), code: code.trim(), language, tags, isPublic });
            setSaved(true);
    ...
    const inputStyle = (hasError) => ({
        ...
        fontSize: ".75rem", color: "var(--text)", outline: "none", transition: "border-color .2s",
        letterSpacing: ".03em",
    });

    const codeLines = code ? code.split("\n").length : 0;
    const hasChanges = title || code || description || tags.length > 0;
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <motion.div {...fadeUp(0.1)}>
                        <Field label="title *" error={errors.title}>
                            <input
                                ...
                                placeholder="e.g. Hash password with bcrypt"
                                maxLength={100}
                    ...
                    <motion.div {...fadeUp(0.2)} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
                        ...
                        <Field label="visibility">
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: ".75rem 1rem", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 7, height: 42 }}>
                                <div>
                        ...
                        <Field label="code *" error={errors.code}>
                            <div style={{ border: `1px solid ${errors.code ? "#e06c75" : "var(--border)"}`, borderRadius: 8, overflow: "hidden", transition: "border-color .2s" }}>
                                ...
                                <textarea
                                    ...
                                    spellCheck={false}
                                    style={{
                                        width: "100%", minHeight: 280, padding: "1rem", border: "none",
                                        background: "var(--bg)", fontFamily: "'IBM Plex Mono', monospace",
                                        fontSize: ".73rem", lineHeight: 1.85, color: "var(--accent)",
                                        outline: "none", resize: "vertical", letterSpacing: ".02em",
                                    }}
                                />
                            </div>
                            <div style={{ fontSize: ".6rem", color: "var(--accent)", opacity: .5, textAlign: "right", marginTop: ".35rem" }}>
                    ...
                    <motion.div {...fadeUp(0.35)} style={{ display: "flex", flexDirection: "column", gap: ".6rem" }}>
                        <button type="submit" disabled={submitting || saved}
                            style={{
                                width: "100%", padding: ".9rem", background: saved ? "var(--accent)" : "var(--text)",
                                color: "var(--bg)", border: "none", borderRadius: 5,
                                fontFamily: "'IBM Plex Mono', monospace", fontSize: ".78rem", letterSpacing: ".1em",
                                cursor: submitting || saved ? "default" : "pointer", transition: "background .2s",
                        ...
                        <Link href="/dashboard"
                            style={{
                                display: "block", width: "100%", padding: ".8rem", background: "transparent",
                                color: "var(--accent)", border: "1px solid var(--border)", borderRadius: 5,
                                fontFamily: "'IBM Plex Mono', monospace", fontSize: ".75rem", letterSpacing: ".08em",
                                cursor: "pointer", textAlign: "center", textDecoration: "none", transition: "border-color .2s",
                            }}
                ...
                <div style={{ position: "sticky", top: 70, display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    ...
                    <motion.div {...fadeUp(0.3)}>
                        <div style={{ padding: "1.25rem", border: "1px solid var(--border)", borderRadius: 8, background: "var(--card)" }}>
                            ...
                            {[
                                "Add descriptive tags so you can find snippets fast later.",
                                "Keep titles short and specific — \"JWT verify middleware\" beats \"auth stuff\".",
                                "Public snippets get a shareable link you can send to teammates.",
                            ].map((tip, i) => (
                                <div key={i} style={{ display: "flex", gap: ".6rem", fontSize: ".67rem", color: "var(--accent)", opacity: .85, lineHeight: 1.65, marginBottom: i < 2 ? ".75rem" : 0 }}>
                                    <span style={{ flexShrink: 0, opacity: .5 }}>0{i + 1}</span>
                                    {tip}
                                </div>
                            ))}
                        </div>
